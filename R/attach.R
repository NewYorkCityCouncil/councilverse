...
councilverse_attach <- function() {
  ...
  # councilverse with the package version on the right-hand side
  load_header <- cli::rule(
    left = crayon::bold("Attaching packages"),
    right = paste0("councilverse ", package_version("councilverse"))
  )

  # Return a character string containing the package version for each of councilverse's constituents
...
councilverse_detach <- function() {
  pak <- paste0("package:", c(pkgs, "councilverse"))
  lapply(pak[pak %in% search()], detach, character.only = TRUE)
  invisible()
}

#' List all packages imported by councilverse
#'
#' @export
#'
#' @examples
#' councilverse_packages()
councilverse_packages <- function() {
  # get all imports from councilverse's package description file
  raw <- utils::packageDescription("councilverse")$Imports
